...
int readMediaList(istream& inFile, ostream& errFile, vector<Media*>& mediaLib) {
    ...
    return static_cast<int>(mediaLib.size());
...
void printMoviesByDirector(ofstream& outFile, const vector<Media*>& mediaLib, const string& directorName) {
    ...
    for (const auto& media : mediaLib) {
        if (media->getType() == 'M') {
            const Movie* movie = dynamic_cast<const Movie*>(media);
            if (movie && movie->getKeyName() == directorName) {
                outFile << movie->getTitle() << endl;
            }
        }
    }
...
void printSongsByArtist(ofstream& outFile, const vector<Media*>& mediaLib, const string& artistName) {
    ...
    for (const auto& media : mediaLib) {
        if (media->getType() == 'S') {
            ...
            if (song && song->getKeyName() == artistName) {
...
//
//    if (!found) {
//        outFile << "No stars found for the movie: " << movieTitle << endl;
//    }
//
//    outFile << "++++++++++++++++++++++++++++++++++++++++++++++++++++\n" << endl;
//}
...
void findMoviesByActor(ofstream& outFile, ofstream& errFile, const vector<Media*>& mediaLib, const string& actorName) {
    bool found = false;
    outFile << "++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl;
    outFile << actorName << " appears in the following movie(s):" << endl;

    for (const auto& media : mediaLib) {
        if (media->getType() == 'M') {
            const Movie* movie = dynamic_cast<const Movie*>(media);
            if (movie && find(movie->getStars().begin(), movie->getStars().end(), actorName) != movie->getStars().end()) {
                outFile << movie->getTitle() << endl;
                found = true;
            }
        }
...
void findMediaByKeyName(ofstream& outFile, ofstream& errFile, const vector<Media*>& mediaLib, const string& keyName) {
    ...
    for (const auto& media : mediaLib) {
        if (media->getTitle().find(keyName) != string::npos || media->getKeyName().find(keyName) != string::npos) {
            outFile << media->getTitle() << " (" << media->getYearReleased() << ")" << endl;
            found = true;
        }
    }
...
//void logError(ostream& errFile, const string& errorMessage) {
//    if (errFile) {
//        errFile << "ERROR: " << errorMessage << endl;
//    }
//}
